...
# ╔═╡ fa0f2e6a-60be-11f1-b5ef-a557c9831568
begin
	using UnfoldSim
	using Random
	using LinearAlgebra
...
# ╔═╡ be65fd4f-4cb8-4984-8033-4762ca5beb8f
md"""
# Simulating data

We'll start with simulating two events, e.g. 🚲 and 🤦 fixations
![](https://cloud.wirdreibei.de/apps/files_sharing/publicpreview/8bkN9dPd5PSNH2t?file=/&fileId=4064180&x=3440&y=1440&a=true&etag=74726548c116dc599cfc3547856fb408)
"""

# ╔═╡ 392166d7-5032-4282-b40e-7b7faf2b35df
md"""
### Create the design
We'll use a SingleSubjectDesign, marking two events, repeating them 10x => 20 trials
"""

# ╔═╡ 6aa4c4c9-f9cc-4a65-ab73-32c6d6e96b48
begin
	design = SingleSubjectDesign(
    conditions = Dict(:event => ["🚲", "🤦"])
) |> d -> RepeatDesign(d, 10)

_evts = generate_events(design)
end


# ╔═╡ 0c71baf3-69f6-48f8-963b-24975076b9c0
md"""
### The signal
We'll create a LinearModelComponent, and instead of having real multiple events, we'll just define the 🚲 = -1*🤦 - else we'd need a `SequenceDesign`
"""

# ╔═╡ cb68e9af-6ce1-4f05-88e7-25cb24014dfe
PlutoTeachingTools.question_box(md"Fill in some random numbers for your `erp_groundtruth`. I recommend a length of 7-10 samples")

# ╔═╡ 249a4af1-0323-4fcc-ad9f-e107284bee51
	PlutoTeachingTools.protip(md"For advanced UnfoldSim user, create a SequenceDesign and have two separate erp_groundtruths! It's a bit more bookkeeping later on, so be warned :)","Advanced Task (SequenceDesign)")

# ╔═╡ 4c0140b6-db59-43d9-888f-f918efc68d9b
PlutoTeachingTools.answer_box("`erp_groundtruth = [0, 0.2, 0.5, 0.8, 1.0, 0.5, 0.1, -0.3, -0.1]`")


# ╔═╡ 98142cc4-b263-47c4-a314-ae08178b6343
begin
	erp_groundtruth = # fill in here
		
	# Create component with formula
	signal = LinearModelComponent(
	    basis = erp_groundtruth,
	    formula = @formula(0 ~ 1 + event),
	    β = [1.0, -2]  # 🚲 and 🚲 + 🤦 (= 1 + - 2 = -1)
	)
end

# ╔═╡ d7ccb65f-d3e5-47f3-82b1-f48cb4b028ab
md"""
We next create the data twice, to directly have an epoched dataset (and don't require any dependencies on Unfold.jl :)
"""

# ╔═╡ 7c853d5e-b48c-4d44-91fb-445b5dec75f5
begin
# Simulate continuous and epoched data
data, evts = simulate(
    MersenneTwister(42),
    design,
    signal,
    UniformOnset(offset=5, width=5),
    PinkNoise(;noiselevel=0)
)

data_epoched, _ = simulate(
    MersenneTwister(42),
    design,
    signal,
    UniformOnset(offset=5, width=5),
    PinkNoise(;noiselevel=0); return_epoched=true
)
end;


# ╔═╡ 1757ad4c-eb95-4590-b62e-2f4d42c31037
PlutoTeachingTools.question_box("Plot your data, inspect your events! Do you have overlap in your data?")

# ╔═╡ eb6c1470-7ef1-4439-9da6-3af3855c27b7
md"""
# Modelling the data
Let's define some variables we will require. You could hardcode them :)
"""

# ╔═╡ c98330dc-1e41-416f-a631-fd596f38b628
begin
	n_timepoints =   # how long is the signal to be modelled (best for this notebook: same as length of event-kernel)
	n_events = # how many total events?
	n_unique_events = # how many unique event types?
	n_data = # how many samples in the continuous data?
	
end;

# ╔═╡ 2b5b5b53-0ca7-4a15-ab17-33d185187065
PlutoTeachingTools.answer_box(md"""
							  ```
begin
	n_timepoints = length(erp_groundtruth)  
	n_events = nrow(evts)
	n_unique_events = length(unique(evts.event))
	n_data = length(data)
	
end
```""")

# ╔═╡ 8cc021bb-933e-4276-8db9-70fd42dcf4fe
md"""
# Mass univariate

Let's first do mass univariate modelling. 

1) Create the designmatrix X, spanning `n_events` x `n_predictors`
2) solve $y = Xb$ (e.g. $X^{-1}*y = X^{-1}*X*b = b$)
3) repeat Step 2 for each timepoint
"""

# ╔═╡ d81aae7d-89bc-4104-ab12-2f28a17134ac
md"""
### Step 1)
The designmatrix X could look the following 
```
20×2 Matrix{Int64}:
 1  0
 0  1
...
20×2 Matrix{Int64}:
 ...
 1   1
```
or ... we'll learn more about that in the workshop on coding schemes :)
"""

# ╔═╡ 1b585c95-2f9b-46e9-8e69-a78f98a0545f
question_box(md"Add code to generate the designmatrix!")

# ╔═╡ a29e86a3-a86a-4c05-a11e-e66cb32335d3
X = # add the designmatrix here

# ╔═╡ c44ba69e-7631-48a2-bf62-4a0064bbd9ce
answer_box(md"""```X = Int.([evts.event .== "🚲" evts.event .== "🤦"]) # done!```""")

# ╔═╡ c39b6e38-ae30-49bd-9678-4ab5c3719cd8
md"""
### 2) Solve for one timepoint

Let's solve: $y_1=Xb_1$

For that we'll use pinv - the pseudoinverse
"""

# ╔═╡ 83c663cf-88bc-47ba-af83-63c8ba21f6d2
y₁ = data_epoched[1,:] # select first timepoint

# ╔═╡ 7e72b739-bcfc-496d-bac1-7e2859768a9c
question_box("Solve it!")

# ╔═╡ e70c094d-066a-424c-8933-33fafad461ec
β₁ = # solve it!

# ╔═╡ 07bd2bf4-89b4-4d5c-ab20-88786c055cc2
PlutoTeachingTools.protip("Solving it via `pinv(XX')` would be faster for large problems, and numerically more stable would be to use X\\y ")

# ╔═╡ 050cedca-0f14-4c63-a8d3-a1c9fd0d45a9
md"""
### 3) Repeat it for each timepoint
"""

# ╔═╡ aea72134-3764-40fb-969a-c6dab9374608
PlutoTeachingTools.question_box("Fix the code below and run a linear model for each timepoint")

# ╔═╡ 568be70c-a10d-4b5d-aca3-c245524858d2
begin
	β = Array{Float64}(undef,n_timepoints,2)
	for t = 1:n_timepoints
		β[t,:] = # solve each timepoint!
	end
	series(β')
end

# ╔═╡ 0eea5748-d387-4ba0-ba4e-c485d89fa21a
answer_box("`pinv(X)*data_epoched[t,:]`")

# ╔═╡ deccfd58-8dc6-4b06-9f03-5de139a22c69
md"""
# Overlap correction
"""

# ╔═╡ dc16cf81-c84e-4989-9295-976fdf0bf0e6
md"""
Now the 🆒 bits: **Overlap correction**
![](https://dfzljdn9uc3pi.cloudfront.net/2019/7838/1/fig-2-1x.jpg)
"""

# ╔═╡ 51ce966b-6ef3-48ba-b111-8fa8517b846c
md"""
We need to create this more complex designmatrix `Xdc`.

Let's do it in two steps:
1) create a function to create the staircaise pattern for one predictor
2) create the orange matrix for 🚲 and then the blue for 🤦 and concatenate them

"""

# ╔═╡ 233fbf25-ae72-402a-924e-cc111257b337
md"""
### 1) one predictor staircase function
"""

# ╔═╡ 2972a38c-2254-457f-a593-578dce92ea9d
question_box("Make use of the scaffold below, and put in the correct indexing to receive the staircase pattern. Think about what each dimension actually represents / means!")

# ╔═╡ 3fb876c1-7298-4a0f-883e-d272265dc888
function generate_designmatrix(evts,n_data,n_timepoints)
	Xdc = zeros(Int,n_data,n_timepoints)
...
# ╔═╡ 8814b143-0426-4568-8467-0934dbcd37fd
answer_box("`Xdc[lat+k,k] = 1`")

# ╔═╡ cfe67ff1-fda9-44f7-8a17-c7423a5a5134
Xdc = generate_designmatrix(evts,n_data,n_timepoints)

# ╔═╡ fd97c0a1-e464-46d4-962b-ab4f26105d44
heatmap(Xdc')


# ╔═╡ d8306e37-ffc3-4b95-b432-49b28873cbaa
md"""
### Intermezzo
What happens if we use this designmatrix to fit our data??
"""

# ╔═╡ 5fce375f-ad06-441f-9ed1-bf498d114e2f
lines(pinv(Xdc)*data)

# ╔═╡ d17a2242-9bd5-43b2-bcad-643160db80f2
md"""
### 2) concatenating the predictors
"""

# ╔═╡ 87bf3ccd-df34-4598-97f1-b7e00b751807
begin
	Xdc_collection = []
	
	for evt_name = unique(evts.event)
		evts_subset = evts[evts.event .== evt_name,:]

		_Xdc = generate_designmatrix(evts_subset,n_data,n_timepoints)
		push!(Xdc_collection,_Xdc)
	end
	Xdc_twoevents = Int.(hcat(Xdc_collection...))

end

# ╔═╡ c843a223-e6c8-4c70-9e60-aa7557fee5c6
heatmap(Xdc_twoevents')

# ╔═╡ 6479dabf-8e62-4b27-b5f1-90716aa44e5a
question_box("Looking at this matrix, where is the overlap encoded?")

# ╔═╡ 0c1d5640-5c44-4d2d-b278-bc4ce9714d8c
md"""
## Solving the final model!

Finally, solve it. use the pinv on your designmatrix, multiply it by the data and plot it. save 
"""

# ╔═╡ 4a50eec5-3d3a-490d-a91e-0dea7946e0e8
b_dc = # solve it!

# ╔═╡ 14b180b3-ad45-4942-9c49-72aa04f56182
answer_box("`b_dc = pinv(Xdc_twoevents)*data`")

# ╔═╡ dc978161-fa41-4769-90fb-0d4214d2cafd
lines(b_dc)

# ╔═╡ 3fcd8841-9e6c-400f-9859-b4bfe06c66f3
question_box("Urgh, why are they taped together and not separate like in the no-dc fitting at the beginning? Split them and plot them!")

# ╔═╡ 1ca050c2-fbb6-4aa9-a544-9a4119e97139
answer_box("""
```

f = lines(b_dc[1:n_timepoints])
lines!(b_dc[n_timepoints+1:end])

ax = lines(f.figure[1,2],erp_groundtruth).axis
lines!(ax,erp_groundtruth.*-1)
f 
```""")
...
manifest_format = "2.0"
project_hash = "7a07cdb6e2919a9d939d46b2b7bab6c4440c5703"
